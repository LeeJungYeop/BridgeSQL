...
생성된 데이터 정의서는 `jhgan/ko-sroberta-multitask`로 임베딩되어 ChromaDB에 저장됩니다. 사용자 질문이 입력되면 코사인 유사도 검색 후 Score Fusion(컬럼 점수 + 부모 테이블 점수 합산)을 적용하여 최종 컨텍스트를 LLM에 전달합니다.
...
[입력] 데이터베이스 연결
    ...
    ▼
[Step 1] 데이터 정의서 생성
    IN  : 테이블명·컬럼명·데이터 타입·샘플 데이터
    처리 : LLM이 비즈니스명·설명·검색 키워드 자동 생성
    OUT : + 컬럼별 비즈니스명 / 설명 / 키워드
    │
    ▼
[Step 2] 벡터 임베딩 및 저장
    컬럼 : 컬럼명·데이터 타입·비즈니스명·설명·키워드·샘플 데이터
...
예를 들어, `"통과된 데이터만 골라내줘"`라는 질문이 들어와도, `PASS_YN` 컬럼의 키워드(`통과, 합격, 검사`)가 임베딩에 포함되어 있어 정확히 매칭됩니다.
...
BridgeSQL은 MCP의 세 가지 구성요소를 활용하여 에이전트가 상황에 따라 스스로 경로를 선택할 수 있는 구조로 설계되었습니다.
...
- `bridgesql://table/{테이블명}` → 특정 테이블의 컬럼·타입·비즈니스명·샘플 데이터
...
- 사용자가 결과를 긍정적으로 확인했을 때만 `learn` 호출 제안
...
search_examples
    ...
    execute_sql ◄─────────────┘
         │
         ├─ 실패 → error_context 담아 generate_sql 재호출
         │
         └─ 성공 → 사용자 확인
                      │
                      └─ 긍정 응답 시 learn 제안 (동의할 경우에만 저장)
```
