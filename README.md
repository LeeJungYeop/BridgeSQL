...
더 나아가, 생성된 데이터 정의서의 모든 테이블·컬럼 정보는 임베딩 모델(`ko-sroberta-multitask`, 현재 한국어 최적화)을 통해 벡터로 변환되어 ChromaDB에 저장됩니다. 사용자가 질문을 입력하면, 코사인 유사도를 기반으로 의미적으로 가장 가까운 컬럼 정보들이 검색되어 LLM에 동적으로 전달됩니다.
