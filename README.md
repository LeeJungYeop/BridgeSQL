...
> 자연어로 데이터베이스에 질문하는 NL2SQL 플랫폼 — AI가 데이터 정의서를 자동 생성하고, MCP를 통해 Claude와 직접 연결됩니다.
...
더 나아가, 생성된 데이터 정의서의 모든 테이블·컬럼 정보는 한국어 임베딩 모델(`ko-sroberta-multitask`)을 통해 벡터로 변환되어 ChromaDB에 저장됩니다. 사용자가 질문을 입력하면, 코사인 유사도를 기반으로 의미적으로 가장 가까운 컬럼 정보들이 검색되어 LLM에 동적으로 전달됩니다.

예를 들어, `"지난주에 찍힌 사진 중에서 통과된 데이터만 골라내줘"`라는 질문이 입력되면, `"통과된"`이라는 말이 없어도 `이미지`, `통과`, `날짜` 관련 벡터와 유사도가 높은 컬럼 정보가 우선적으로 검색되어 프롬프트에 포함됩니다.

## Memory & Few-shot

BridgeSQL은 메모리 시스템을 통해 대화가 거듭될수록 더 정확해지는 학습형 엔진으로 진화합니다. 과거에 성공적으로 실행된 질문-SQL 쌍을 메모리에 저장하고, 새로운 질문이 들어오면 가장 유사한 과거 사례를 few-shot 예시로 LLM에 제공합니다.
...
MCP(Model Context Protocol)는 Anthropic이 제안한 개방형 표준으로, Claude 같은 LLM이 외부 도구·데이터와 구조화된 방식으로 통신할 수 있게 해주는 인터페이스입니다.

BridgeSQL은 MCP의 세 가지 구성요소를 모두 활용합니다.
...
기존 방식은 `query`라는 하나의 도구가 RAG 검색, SQL 생성, 실행, 결과 반환을 모두 일괄 처리하는 방식입니다. SQL 실행이 실패했을 때 에이전트가 오류 원인을 분석하고 보정된 SQL을 재시도하거나, 비슷한 과거 사례가 있을 때 RAG 검색을 생략하고 바로 재활용하는 식의 경로 선택이 불가능합니다. `query`라는 단일 도구는 에이전트에게 길을 하나만 남겨둔 셈입니다. 이 문제를 해결하기 위해 MCP의 세 가지 구성요소를 활용하여 에이전트가 스스로 경로를 선택할 수 있는 구조로 재설계했습니다.

### Tools

에이전트가 직접 호출하여 파이프라인을 구성하는 핵심입니다. 다섯 가지 도구로 분해되어 있으며, 에이전트는 상황에 따라 이를 조합하여 사용합니다.
...
| `execute_sql` | SQL 실행 |
| `search_examples` | 저장된 성공 사례 검색 |
| `learn` | 확인된 질문-SQL 쌍을 메모리에 저장 |
| `examples_delete` | 잘못된 예제 삭제 |
