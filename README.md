...
더 나아가, 생성된 데이터 정의서는 한국어 임베딩 모델(`jhgan/ko-sroberta-multitask`)을 통해 벡터로 변환되어 ChromaDB에 저장됩니다. 컬럼·테이블을 각각 독립 문서로 임베딩하며, 각 문서에는 테이블명·컬럼명·데이터 타입·제약조건·비즈니스명·설명·키워드·샘플값이 함께 포함됩니다. 사용자 질문이 입력되면 동일 모델로 임베딩한 뒤 코사인 유사도 기반 검색을 수행하고, 컬럼 점수에 부모 테이블 점수를 가산하는 Score Fusion을 적용하여 최종 컨텍스트를 LLM에 전달합니다.
...
[Step 1] 데이터 정의서 생성
    ...
    ▼
[Step 2] 벡터 임베딩 및 저장
    IN  : 테이블명·컬럼명·데이터 타입·제약조건·비즈니스명·설명·키워드·샘플값
    모델 : jhgan/ko-sroberta-multitask
    단위 : 컬럼·테이블을 각각 독립 문서로 임베딩
...
[Step 2] 유사도 검색 + Score Fusion
    IN  : 질문 벡터
    방법 : 코사인 유사도 top-k → 컬럼 점수 + 부모 테이블 점수 boost
    OUT : 관련 컬럼·테이블 컨텍스트
    │
    ▼
