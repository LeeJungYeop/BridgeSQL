...
더 나아가, 생성된 데이터 정의서는 한국어 임베딩 모델(`jhgan/ko-sroberta-multitask`)을 통해 벡터로 변환되어 ChromaDB에 저장됩니다. 임베딩 단위는 컬럼·테이블 각각이며, 각 문서에는 물리명·비즈니스명·설명·키워드·샘플값이 함께 포함됩니다. 사용자 질문이 입력되면 동일 모델로 임베딩한 뒤 코사인 유사도 기반 검색을 수행하고, 컬럼 점수에 부모 테이블 점수를 가산하는 Score Fusion을 적용하여 최종 컨텍스트를 LLM에 전달합니다.

예를 들어, `"지난주에 찍힌 사진 중에서 통과된 데이터만 골라내줘"`라는 질문이 입력되면, `"통과된"`이라는 말이 없어도 `이미지`, `통과`, `날짜` 관련 벡터와 유사도가 높은 컬럼 정보가 우선적으로 검색되어 프롬프트에 포함됩니다.

## Memory & Few-shot

BridgeSQL은 메모리 시스템을 통해 대화가 거듭될수록 더 정확해질 수 있는 구조로 설계되어 있습니다. 과거에 성공적으로 실행된 질문-SQL 쌍을 메모리에 저장하고, 새로운 질문이 들어오면 가장 유사한 과거 사례를 few-shot 예시로 LLM에 제공합니다.
